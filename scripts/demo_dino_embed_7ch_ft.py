...
# ---------------------------------------------------------------------------
# Contrastive finetuning — SimCLR
# ---------------------------------------------------------------------------

def _augment_pair(img7: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    Returns (original, augmented) as a positive pair.

    Augmentation: random rotation in the XY image plane by θ ∈ [0°, 360°).

    Spatial rotation (all 7 channels) is applied with scipy, then the XY
    components of the two normal triplets are rotated by the same angle so
    that encoded directions remain geometrically consistent:

      For each normal triplet (ch x, ch y, ch z):
        decode  : n = ch * 2 − 1         (→ [-1, 1])
        rotate  : Nx' = cos θ · Nx − sin θ · Ny
                  Ny' = sin θ · Nx + cos θ · Ny
                  Nz' = Nz               (Z-axis rotation leaves Nz unchanged)
        re-encode: ch = clip((n' + 1) / 2, 0, 1)

    Depth channel (ch 6) only needs the spatial rotation; values are unchanged.
    """
    theta     = np.random.uniform(0.0, 360.0)
    theta_rad = np.deg2rad(theta)
    cos_t, sin_t = np.cos(theta_rad), np.sin(theta_rad)

    aug = nd_rotate(img7, angle=theta, axes=(0, 1), reshape=False,
                    mode="constant", cval=0.0).astype(np.float32)
    aug = np.clip(aug, 0.0, 1.0)

    for x_ch, y_ch in ((0, 1), (3, 4)):   # top normals, bottom normals
        nx = aug[:, :, x_ch] * 2.0 - 1.0
        ny = aug[:, :, y_ch] * 2.0 - 1.0
        aug[:, :, x_ch] = np.clip((cos_t * nx - sin_t * ny + 1.0) * 0.5, 0.0, 1.0)
        aug[:, :, y_ch] = np.clip((sin_t * nx + cos_t * ny + 1.0) * 0.5, 0.0, 1.0)
...
def _img7_to_tensor(img7: np.ndarray, device: torch.device, img_size: int) -> torch.Tensor:
    ...
    if sq != img_size:
        ...
        img7 = np.stack(channels, axis=2)
    t = torch.from_numpy(img7.transpose(2, 0, 1)).unsqueeze(0).to(device)
    return t * 2.0 - 1.0   # [1, 7, H, W] in [-1, 1]


class ProjectionHead(nn.Module):
    """2-layer MLP that maps DINO CLS token → unit-normalised projection space."""
...
class NTXentLoss(nn.Module):
    ...
    For each i the positive pair is (z1[i], z2[i]).
    All other 2N-2 samples in the concatenated batch are negatives.
    """
    ...
    def forward(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        ...
        # mask self-similarity so each sample is not its own negative
        sim.masked_fill_(torch.eye(2 * N, dtype=torch.bool, device=z.device), float("-inf"))

        # label i → i+N  and  i+N → i
...
def finetune_simclr(
    ...
    """
    Finetune the 7-channel DINOv2 with SimCLR contrastive loss.

    Positive pair  : (original render, axis-aligned render) of the same stone.
    Negative pairs : all cross-stone combinations within the batch.

    Returns the model with updated weights (projection head is discarded after
    training; use the raw CLS token for final embeddings).
    """
    _set_trainable(model, n_unfreeze_blocks)
    model.train()

    head = ProjectionHead(in_dim=model.embed_dim).to(device)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        log.info(f"  epoch {epoch:>3}/{n_epochs}  loss={avg:.4f}")
...
def main():
    ...
    renders_dir = ensure_dir(RENDERS_DIR)
    stems: list[str] = []

    # Pass 2 — render every mesh to the shared renders dir (skip if already present)
    for i, path in enumerate(files, 1):
        log.info(f"[{i}/{len(files)}] {path.stem}")
        top_png = renders_dir / f"{path.stem}_top.png"
        if top_png.exists():
    ...
    for stem in stems:
        ...
        log.info(f"  {stem}  embed dim={emb.shape[0]}")
