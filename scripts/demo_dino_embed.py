...
Each 224×224 image is passed through DINOv2-ViT-S/14 (lightest variant).
The three 384-d CLS tokens are concatenated → 1152-d embedding per stone.
...
Usage
-----
  python scripts/demo_dino_embed.py
  python scripts/demo_dino_embed.py --save_renders
  python scripts/demo_dino_embed.py --skip_clean
  python scripts/demo_dino_embed.py --decimate --target_ratio 0.05
  python scripts/demo_dino_embed.py --pca_only    # replot cached embeddings
"""
...
def load_dino(model_name: str = DINO_MODEL, device: torch.device = None):
    """Load DINOv2 from torch.hub (downloads weights on first run ~86 MB)."""
...
@torch.no_grad()
def embed_images(model, imgs: list[np.ndarray], device: torch.device) -> np.ndarray:
    """[H, W, 3] uint8 × N  →  [N, D] float32 CLS-token embeddings."""
    batch = torch.stack([_DINO_TRANSFORM(Image.fromarray(im)) for im in imgs]).to(device)
...
def plot_pca(stems, embeddings, out, site_map=None, notes_map=None):
    ...
    _plot_2d(stems, coords, site_map, notes_map,
             f"DINOv2 embeddings — PCA  ({len(stems)} stones)",
...
def plot_pca_3d(stems, embeddings, out, site_map=None, notes_map=None):
    ...
    fig = _plotly_3d(stems, coords, axis_labels,
                     f"DINOv2 embeddings — PCA 3D  ({len(stems)} stones)",
                     site_map, notes_map)
...
def plot_umap(stems, embeddings, out, site_map=None, notes_map=None, seed=42):
    ...
    _plot_2d(stems, c2, site_map, notes_map,
             f"DINOv2 embeddings — UMAP  ({len(stems)} stones)",
    ...
    try:
        ...
        fig3 = _plotly_3d(stems, c3, ["UMAP-1", "UMAP-2", "UMAP-3"],
                          f"DINOv2 embeddings — UMAP 3D  ({len(stems)} stones)",
                          site_map, notes_map)
...
def parse_args():
    ...
    p.add_argument("--save_renders",   action="store_true",
                   help="Save rendered PNG images to outputs/dino/renders/")
...
def main():
    ...
    # Load DINOv2
    model = load_dino(args.dino_model, device=device)
    D     = model.embed_dim
    log.info(f"DINOv2 {args.dino_model}  embed_dim={D}  → 3×{D}={3*D}d per stone")
    ...
    for i, path in enumerate(files, 1):
        ...
        if emb_path.exists():
            ...
            try:
                ...
                feats = embed_images(model, [top_n, bot_n, depth], device)  # [3, D]
                emb   = feats.flatten()                                       # [3D]
